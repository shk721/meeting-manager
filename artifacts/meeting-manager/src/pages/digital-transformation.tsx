...
// ─── API helpers ──────────────────────────────────────────────────────────────
async function dtFetch(path: string, opts?: RequestInit) {
  const res = await fetch(path, { credentials: "include", ...opts });
  if (!res.ok) throw new Error(`HTTP ${res.status}`);
  if (res.status === 204) return null;
  return res.json();
}
function dtPost(path: string, body: unknown) {
  return dtFetch(path, { method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify(body) });
}
function dtPatch(path: string, body: unknown) {
  return dtFetch(path, { method:"PATCH", headers:{"Content-Type":"application/json"}, body: JSON.stringify(body) });
}
function dtDelete(path: string) { return dtFetch(path, { method:"DELETE" }); }

const api = {
  listProjects: () => dtFetch("/api/dt-projects"),
  getProject: (id: number) => dtFetch(`/api/dt-projects/${id}`),
  createProject: (d: { title: string; deadline: string }) => dtPost("/api/dt-projects", d),
  updateProject: (id: number, d: unknown) => dtPatch(`/api/dt-projects/${id}`, d),
  deleteProject: (id: number) => dtDelete(`/api/dt-projects/${id}`),
  createSubplan: (pid: number, d: unknown) => dtPost(`/api/dt-projects/${pid}/subplans`, d),
  updateSubplan: (id: number, d: unknown) => dtPatch(`/api/dt-subplans/${id}`, d),
  deleteSubplan: (id: number) => dtDelete(`/api/dt-subplans/${id}`),
  createResource: (sid: number, d: unknown) => dtPost(`/api/dt-subplans/${sid}/resources`, d),
  updateResource: (id: number, d: unknown) => dtPatch(`/api/dt-resources/${id}`, d),
  deleteResource: (id: number) => dtDelete(`/api/dt-resources/${id}`),
  createComponent: (sid: number, d: unknown) => dtPost(`/api/dt-subplans/${sid}/components`, d),
  updateComponent: (id: number, d: unknown) => dtPatch(`/api/dt-components/${id}`, d),
  deleteComponent: (id: number) => dtDelete(`/api/dt-components/${id}`),
  createTask: (cid: number, d: unknown) => dtPost(`/api/dt-components/${cid}/tasks`, d),
  updateTask: (id: number, d: unknown) => dtPatch(`/api/dt-tasks/${id}`, d),
  deleteTask: (id: number) => dtDelete(`/api/dt-tasks/${id}`),
  addTaskUpdate: (tid: number, d: unknown) => dtPost(`/api/dt-tasks/${tid}/updates`, d),
  listSnapshots: (pid: number) => dtFetch(`/api/dt-projects/${pid}/snapshots`),
  createSnapshot: (pid: number, d: unknown) => dtPost(`/api/dt-projects/${pid}/snapshots`, d),
  deleteSnapshot: (id: number) => dtDelete(`/api/dt-snapshots/${id}`),
};
...
function adaptComp(c: any): Comp {
  return { id: String(c.id), driver: c.driver, title: c.title, desc: c.desc, priority: c.priority, refYear: c.refYear, tasks: (c.tasks || []).map(adaptTask) };
}
function adaptSubplan(sp: any): SubPlan {
  return { id: String(sp.id), title: sp.title, status: sp.status, progress: sp.progress, deadline: sp.deadline,
    resources: (sp.resources || []).map((r: any) => ({ id: String(r.id), name: r.name, role: r.role, allocation: r.allocation })),
    components: (sp.components || []).map(adaptComp) };
}
function adaptProject(p: any): Project {
  return { id: p.id, title: p.title, deadline: p.deadline, subplans: (p.subplans || []).map(adaptSubplan) };
}
function adaptSnapshot(s: any): Snapshot {
  return { id: String(s.id), label: s.label, period: (s.period || "manual") as Snapshot["period"], createdAt: s.createdAt, metrics: s.metrics as unknown as Metrics };
}
...
function computeMetrics(project: Project): Metrics {
  ...
  return {
    overallProgress, totalSubplans: project.subplans.length,
    ...
    subplanSummaries: project.subplans.map(sp => {
      ...
      return { id: sp.id, title: sp.title, progress: sp.progress, status: sp.status, tasksTotal: tasks.length, tasksDone: tasks.filter(t => t.status === "مكتمل").length };
    }),
  };
}
...
function fmtDate(iso: string) {
  ...
  return `${d.toLocaleDateString("ar-SA", { day:"2-digit", month:"short", year:"numeric" })} — ${d.toLocaleTimeString("ar-SA", { hour:"2-digit", minute:"2-digit" })}`;
}
  ...
  return (
    <svg width={size} height={size} style={{ transform:"rotate(-90deg)" }}>
      ...
      <circle cx={size/2} cy={size/2} r={r} fill="none" stroke={color} strokeWidth={stroke}
        strokeDasharray={circ} strokeDashoffset={off} strokeLinecap="round" style={{ transition:"stroke-dashoffset .4s" }}/>
    </svg>
  );
}
  ...
  return (
    <button onClick={onClick} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      style={{ background:s.bg, color:s.color, border:s.border, borderRadius:7, cursor:"pointer",
        ...
        transition:"all .15s", whiteSpace:"nowrap", transform: hov && !active ? "translateY(-1px)" : "none" }}>
      {children}
    </button>
  );
}
...
function Sel({ value, onChange, options }: {
  ...
  return (
    <select value={value} onChange={e => onChange(e.target.value)} style={{
      ...
      color:C.text, padding:"7px 11px", fontSize:13, outline:"none", fontFamily:"inherit", direction:"rtl", width:"100%",
    }}>
      ...
      <div style={{ borderTop:`1px solid ${C.border}`, paddingTop:16 }}>
        ...
        {logs.length === 0
          ...
          : [...logs].reverse().map((log, i) => (
            <div key={i} style={{ display:"flex", gap:12, padding:"10px 0", borderBottom:`1px solid ${C.border}`, opacity: i > 0 ? 0.75 : 1 }}>
              <div style={{ display:"flex", flexDirection:"column", alignItems:"center", flexShrink:0 }}>
                <div style={{ width:8, height:8, borderRadius:"50%", marginTop:4, background: i === 0 ? C.accent : C.muted }}/>
                {i < logs.length - 1 && <div style={{ flex:1, width:1, background:C.border, marginTop:4 }}/>}
              </div>
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ fontSize:11, color:C.muted, marginBottom:4 }}>
                  🕐 {fmtDate(log.at)}{log.by && <span style={{ marginRight:8, color:C.sub }}>· {log.by}</span>}
                </div>
                <div style={{ fontSize:12, color:C.text, lineHeight:1.7, background:C.raised, borderRadius:7, padding:"8px 10px" }}>{log.note}</div>
              </div>
            </div>
          ))
        }
...
function TaskRow({ task: t, logCount, hasManual, onStatusChange, onOpenLog, onDelete }: {
  ...
  return (
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      ...
      <div style={{ display:"flex", alignItems:"center", gap:SP.sm, padding:"9px 12px" }}>
        <div style={{ flex:1, minWidth:0 }}>
          ...
          <div style={{ display:"flex", gap:SP.md, marginTop:3, flexWrap:"wrap", alignItems:"center" }}>
            ...
            {t.updatedAt && hov && (
              <span style={{ fontSize:10, color:C.muted }}>🕐 {new Date(t.updatedAt).toLocaleDateString("ar-SA", { day:"2-digit", month:"short" })}</span>
            )}
          </div>
        </div>
        <div style={{ minWidth:120 }}><Sel value={t.status} onChange={onStatusChange} options={STATUS_KEYS}/></div>
      ...
      {lastNote && (
        <div style={{ borderTop:`1px solid ${C.border}`, padding:"6px 12px", background:`${C.accent}08`, display:"flex", gap:SP.sm, alignItems:"flex-start" }}>
...
function ComponentCard({ comp, onUpdateComp, onDeleteComp }: {
  comp: Comp; onUpdateComp: (updated: Comp) => void; onDeleteComp: (cid: string) => void;
}) {
  ...
  const setTaskStatus = async (tid: string, status: string) => {
    ...
    try {
      const updated = await api.updateTask(toNum(tid), { status });
      await api.addTaskUpdate(toNum(tid), { note: autoNote, auto: "true" });
      const ts = nowISO();
      const tasks = comp.tasks.map(t => {
        if (t.id !== tid) return t;
        return { ...adaptTask(updated), updates: [...(t.updates || []), { at: ts, note: autoNote, auto: true }] };
      });
  ...
  const addUpdate = async (tid: string, note: string) => {
    try {
      const upd = await api.addTaskUpdate(toNum(tid), { note });
      const tasks = comp.tasks.map(t => {
        if (t.id !== tid) return t;
        return { ...t, updatedAt: nowISO(), updates: [...(t.updates || []), { at: upd.createdAt, note }] };
      });
      onUpdateComp({ ...comp, tasks });
  ...
  const deleteTask = async (tid: string) => {
    try {
      await api.deleteTask(toNum(tid));
  ...
  const submitTask = async () => {
    ...
    try {
      const created = await api.createTask(toNum(comp.id), { title: newTask.title, status: newTask.status, assignee: newTask.assignee });
      onUpdateComp({ ...comp, tasks: [...comp.tasks, adaptTask(created)] });
  ...
  return (
    <>
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      style={{ background: hov ? C.card : C.raised, border:`1px solid ${hov ? (d?.color + "55" || C.sub) : C.border}`,
        borderRadius:11, marginBottom:10, overflow:"hidden",
        borderRight:`3px solid ${d?.color || C.muted}`, transition:"all .2s", boxShadow: hov ? "0 4px 16px #00000030" : "none" }}>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", gap:8, flexWrap:"wrap" }}>
            ...
            <div style={{ display:"flex", gap:6, alignItems:"center", flexShrink:0 }}>
              ...
              <span style={{ fontSize:11, fontWeight:700, color:pr }}>{comp.priority==="عالية"?"●●●":comp.priority==="متوسطة"?"●●○":"●○○"}</span>
          ...
          <div style={{ marginTop:4 }}>
            ...
            {comp.tasks.map(t => (
              <TaskRow key={t.id} task={t}
                logCount={(t.updates || []).length} hasManual={(t.updates || []).some(u => !u.auto)}
                onStatusChange={v => setTaskStatus(t.id, v)}
                onOpenLog={() => setLogTask(t)} onDelete={() => deleteTask(t.id)}/>
            ))}
            {addingTask && (
              <div style={{ marginTop:10, display:"flex", flexDirection:"column", gap:8, background:C.surface, borderRadius:8, padding:12 }}>
    ...
    {logTask && (
      <UpdateLogModal
        ...
        onAddUpdate={addUpdate} onClose={() => setLogTask(null)}/>
    )}
    </>
  );
}
...
function ComponentForm({ initial, onSave, onClose }: {
  ...
  const [f, setF] = useState<Omit<Comp, "id"|"tasks">>({
    driver: initial?.driver || "challenge", title: initial?.title || "",
    desc: initial?.desc || "", priority: initial?.priority || "عالية",
            ...
            {DRIVER_KEYS.map(k => {
              ...
              return (
                <button key={k} onClick={() => setF(p => ({ ...p, driver:k }))} style={{
                  ...
                  border:`1px solid ${f.driver===k?d.color:C.border}`, borderRadius:9, padding:"9px 12px",
...
function SubplanDetail({ sp, onUpdateSP }: { sp: SubPlan; onUpdateSP: (updated: SubPlan) => void }) {
  ...
  const updateComp = useCallback((updated: Comp) => {
    ...
    if (recalc.progress !== sp.progress || recalc.status !== sp.status) {
      api.updateSubplan(toNum(sp.id), { progress: recalc.progress, status: recalc.status }).catch(() => {});
    }
  }, [sp, onUpdateSP]);

  const deleteComp = useCallback(async (cid: string) => {
    try {
      await api.deleteComponent(toNum(cid));
      const components = sp.components.filter(c => c.id !== cid);
      const recalc = recalcSP({ ...sp, components });
      onUpdateSP(recalc);
      await api.updateSubplan(toNum(sp.id), { progress: recalc.progress, status: recalc.status });
    } catch {}
  }, [sp, onUpdateSP]);

  const saveComp = async (data: Omit<Comp, "id"|"tasks">) => {
    try {
      const created = await api.createComponent(toNum(sp.id), data);
      onUpdateSP({ ...sp, components: [...sp.components, adaptComp(created)] });
    } catch {}
  };
  ...
  const submitRes = async () => {
    ...
    try {
      const created = await api.createResource(toNum(sp.id), newRes);
  ...
  const removeRes = async (rid: string) => {
    try {
      await api.deleteResource(toNum(rid));
  ...
  const applyProg = async () => {
    ...
    const status = progress === 100 ? "مكتمل" : progress > 0 ? "جاري التنفيذ" : "لم يبدأ";
    try {
      await api.updateSubplan(toNum(sp.id), { progress, status });
    ...
    <div>
      ...
      <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:13, padding:18, marginBottom:14 }}>
        ...
        {sp.components.length === 0
          ...
          : sp.components.map(c => (
              <ComponentCard key={c.id} comp={c} onUpdateComp={updateComp} onDeleteComp={deleteComp}/>
            ))
        }
      </div>
      ...
      <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:13, padding:18 }}>
        ...
        {sp.resources.length === 0
          ...
          : sp.resources.map(r => (
            <div key={r.id} style={{ display:"flex", alignItems:"center", gap:11, padding:"9px 0", borderBottom:`1px solid ${C.border}` }}>
              <div style={{ width:34, height:34, borderRadius:"50%", background:`${C.accent}22`,
                display:"flex", alignItems:"center", justifyContent:"center", fontSize:14, color:C.accent, fontWeight:700, flexShrink:0 }}>{r.name[0]}</div>
              <div style={{ flex:1 }}>
              ...
              <button onClick={() => removeRes(r.id)} style={{ background:"none", border:"none", color:C.muted, cursor:"pointer", fontSize:14 }}>✕</button>
            </div>
          ))
        }
        {addRes && (
          <div style={{ marginTop:12, background:C.surface, borderRadius:9, padding:14, display:"flex", flexDirection:"column", gap:10 }}>
            ...
            <div>
              ...
              <input type="range" min={10} max={100} step={10} value={newRes.allocation}
                onChange={e => setNewRes(p => ({ ...p, allocation:+e.target.value }))} style={{ width:"100%", accentColor:C.accent }}/>
            </div>
            <div style={{ display:"flex", gap:7 }}>
...
// ─── Overview ─────────────────────────────────────────────────────────────────
function HoverCard({ onClick, style, children }: { onClick?: () => void; style?: React.CSSProperties; children: React.ReactNode; }) {
  const [hov, setHov] = useState(false);
  return (
    ...
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:8 }}>
        <div style={{ flex:1, minWidth:0 }}>
          ...
          <div style={{ fontSize:11, color:C.muted, marginTop:3 }}>
            ...
            <span style={{ marginRight:8, background:`${C.accent}18`, color:C.accent, border:`1px solid ${C.accent}30`, borderRadius:5, padding:"1px 7px", fontSize:10 }}>
        ...
        <div style={{ display:"flex", gap:6, flexShrink:0 }}>
          <button onClick={e => { e.stopPropagation(); onExport(); }}
            style={{ background:"transparent", border:`1px solid ${C.border}`, borderRadius:6, cursor:"pointer", padding:"4px 8px", fontSize:11, color:C.sub }}>⬇ JSON</button>
          <button onClick={e => { e.stopPropagation(); onDelete(); }}
            style={{ background:"#ef444418", border:"1px solid #ef444440", borderRadius:6, cursor:"pointer", padding:"4px 8px", fontSize:11, color:"#ef4444" }}>✕</button>
        </div>
      </div>
      ...
      <div style={{ display:"flex", gap:6, marginTop:10, flexWrap:"wrap" }}>
        {DRIVER_KEYS.map(k => {
          ...
          const d = DRIVER[k];
          return (
            <span key={k} style={{ fontSize:10, background:`${d.color}18`, color:d.color, border:`1px solid ${d.color}30`, borderRadius:20, padding:"2px 8px" }}>
              {d.icon} {cnt}
            </span>
          );
        })}
          ...
          {m.subplanSummaries.map(s => {
            ...
            return (
              <div key={s.id} style={{ display:"flex", alignItems:"center", gap:8, marginBottom:6 }}>
                ...
                <span style={{ fontSize:10, color:C.sub, flexShrink:0, maxWidth:130, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{s.title}</span>
    ...
    <div>
      <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:13, padding:18, marginBottom:16 }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(110px,1fr))", gap:10 }}>
          {[
            ...
            <div key={item.label} style={{ background:C.raised, border:`1px solid ${C.border}`, borderRadius:10, padding:"12px 14px", textAlign:"center" }}>
      ...
      <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:13, padding:18, marginBottom:16 }}>
        ...
        <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
          ...
          <Btn onClick={() => { onAddSnapshot(newLabel, newPeriod); setNewLabel(""); }} active>📸 حفظ لقطة</Btn>
          {snapshots.length > 0 && (
            <Btn onClick={() => exportJSON({ project, snapshots, exportedAt: nowISO() }, `dt-report-${Date.now()}.json`)} variant="ghost">⬇ تصدير الكل JSON</Btn>
          )}
        </div>
      </div>
      ...
      <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:13, padding:18 }}>
        <div style={{ fontSize:14, fontWeight:800, color:C.text, marginBottom:14 }}>سجل اللقطات ({snapshots.length})</div>
        {snapshots.length === 0
...
export default function DigitalTransformationPage() {
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        const snaps = await api.listSnapshots(proj.id);
  ...
  const addSP = async (title: string, deadline: string) => {
    if (!project) return;
    try {
      const created = await api.createSubplan(project.id, { title, deadline });
      setProject(prev => prev ? { ...prev, subplans: [...prev.subplans, adaptSubplan(created)] } : null);
  ...
  const removeSP = async (id: string) => {
    try {
      await api.deleteSubplan(toNum(id));
  ...
  const handleAddSnapshot = async (label: string, period: Snapshot["period"]) => {
    ...
    try {
      ...
      const created = await api.createSnapshot(project.id, { label: finalLabel, period, metrics: metrics as unknown as Record<string, unknown> });
  ...
  const handleDeleteSnapshot = async (id: string) => {
    try {
      await api.deleteSnapshot(toNum(id));
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.bg, color:C.text, fontFamily:"'Cairo','Segoe UI',sans-serif", direction:"rtl" }}>
      ...
      <div style={{ background:C.surface, borderBottom:`1px solid ${C.border}`,
        ...
        <button onClick={() => setSidebarOpen(o => !o)}
          style={{ background:"none", border:`1px solid ${C.border}`, borderRadius:6, color:C.muted, cursor:"pointer", fontSize:14, padding:"4px 8px", flexShrink:0 }}>☰</button>
        ...
        <div style={{ display:"flex", gap:6 }}>
          ...
          <a href="/governance" style={{ textDecoration:"none" }}><Btn sm variant="ghost">🏛️ الحوكمة</Btn></a>
        </div>
      ...
      {/* Layout */}
      <div style={{ display:"grid", gridTemplateColumns: sidebarOpen ? "248px 1fr" : "0px 1fr", minHeight:"calc(100vh - 60px)", transition:"grid-template-columns .25s" }}>

        {/* Sidebar */}
        <div style={{ background:C.surface, borderLeft:`1px solid ${C.border}`, overflowY:"auto", overflowX:"hidden",
          ...
          <div style={{ padding:"0 13px 8px" }}>
            <input value={subplanSearch} onChange={e => setSubplanSearch(e.target.value)} placeholder="بحث…"
              style={{ width:"100%", background:C.raised, border:`1px solid ${C.border}`, borderRadius:6,
                color:C.text, padding:"5px 9px", fontSize:12, fontFamily:"inherit", direction:"rtl", outline:"none", boxSizing:"border-box" }}/>
          </div>

          {project.subplans
            .filter(sp => !subplanSearch.trim() || sp.title.toLowerCase().includes(subplanSearch.trim().toLowerCase()))
            .map((sp, i) => {
              ...
              return (
                <div key={sp.id} onClick={() => { setSelId(sp.id); setView("detail"); }}
                  ...
                  <div style={{ display:"flex", gap:8, alignItems:"flex-start" }}>
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ fontSize:12, fontWeight:600, color:active?C.text:C.sub, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{sp.title}</div>
                    ...
                    <button onClick={e => { e.stopPropagation(); removeSP(sp.id); }}
                      style={{ background:"none", border:"none", color:C.muted, cursor:"pointer", fontSize:12, padding:0, opacity:.5, flexShrink:0, marginTop:1 }}>✕</button>
                  </div>
                </div>
              );
            })
          ...
          {snapshots.length > 0 && (
            <div style={{ margin:"8px 13px 0", padding:"8px 12px", background:`${C.accent}10`, border:`1px solid ${C.accent}25`, borderRadius:8, cursor:"pointer" }}
        ...
        <div style={{ padding:20, overflowY:"auto" }}>
          {view === "reports"
            ? <ReportsView project={project} snapshots={snapshots} onAddSnapshot={handleAddSnapshot} onDeleteSnapshot={handleDeleteSnapshot}/>
