...
interface Task {
  ...
  updates: UpdateLog[]; createdAt: string; updatedAt: string;
}

interface Comp {
  id: string; driver: string; title: string; desc: string;
  priority: string; refYear: number; tasks: Task[];
}

interface Resource { id: string; name: string; role: string; allocation: number; }

interface SubPlan {
  id: string; title: string; status: string; progress: number; deadline: string;
  resources: Resource[]; components: Comp[];
}

// ─── Storage ──────────────────────────────────────────────────────────────────
const STORE_KEY = "dt-project-v1";
const SNAP_KEY  = "dt-snapshots-v1";
...
interface Metrics {
  ...
  subplanSummaries: Array<{
    id: string; title: string; progress: number;
    status: string; tasksTotal: number; tasksDone: number;
  }>;
}
...
interface Snapshot {
  id: string;
  label: string;
  period: "manual" | "monthly" | "quarterly";
  createdAt: string;
  projectTitle: string;
  metrics: Metrics;
}
...
function computeMetrics(project: Project): Metrics {
  ...
  const overallProgress = project.subplans.length
    ? Math.round(project.subplans.reduce((s, p) => s + p.progress, 0) / project.subplans.length)
    : 0;
  ...
  return {
    ...
    subplanSummaries: project.subplans.map(sp => {
      ...
      return {
        id: sp.id, title: sp.title, progress: sp.progress,
        status: sp.status, tasksTotal: tasks.length,
        tasksDone: tasks.filter(t => t.status === "مكتمل").length,
      };
    }),
  };
}

function createSnapshot(project: Project, label: string, period: Snapshot["period"] = "manual"): Snapshot {
  return {
    id: uid(),
    label: label || `لقطة ${fmtDate(new Date().toISOString())}`,
    period,
    createdAt: new Date().toISOString(),
    projectTitle: project.title,
    metrics: computeMetrics(project),
  };
}
...
interface Project { title: string; deadline: string; subplans: SubPlan[]; }

const SEED: Project = {
  title: "خطة التحول الرقمي 2025",
  deadline: "2025-12-31",
  subplans: [
    {
      id:"sp1", title:"تحديث منصة التجزئة الرقمية",
      status:"جاري التنفيذ", progress:58, deadline:"2025-09-30",
      resources:[
        {id:"r1",name:"أحمد محمد",role:"مدير مشروع",allocation:100},
        {id:"r2",name:"سارة علي",role:"مطور",allocation:80},
      ],
      components:[
        { id:"c1", driver:"challenge",
          title:"معدل التخلي عن عربة التسوق 42%",
          desc:"تحليل 2024 كشف أن معظم العملاء يتركون العملية عند خطوة الدفع بسبب تعقيد الواجهة",
          priority:"عالية", refYear:2024,
          tasks:[
            {id:"t1",title:"إعادة تصميم مسار الدفع",status:"جاري التنفيذ",assignee:"سارة علي",updates:[],createdAt:"",updatedAt:""},
            {id:"t2",title:"تبسيط نموذج التسجيل",status:"مكتمل",assignee:"سارة علي",updates:[],createdAt:"",updatedAt:""},
          ]
        },
        { id:"c2", driver:"opportunity",
          title:"120 ألف عميل خامل قابل للتفعيل",
          desc:"قاعدة بيانات عملاء لم تُستثمر، يمكن استهدافها بحملات مخصّصة لرفع الإيراد 18%",
          priority:"متوسطة", refYear:2024,
          tasks:[
            {id:"t3",title:"بناء محرك التوصيات الشخصية",status:"لم يبدأ",assignee:"أحمد محمد",updates:[],createdAt:"",updatedAt:""},
          ]
        },
        { id:"c3", driver:"initiative",
          title:"برنامج الولاء متعدد القنوات",
          desc:"مبادرة جديدة لدمج نقاط الولاء عبر التطبيق والمتجر الفيزيائي لزيادة الاحتفاظ",
          priority:"متوسطة", refYear:2025,
          tasks:[
            {id:"t4",title:"تصميم نظام النقاط",status:"لم يبدأ",assignee:"سارة علي",updates:[],createdAt:"",updatedAt:""},
            {id:"t5",title:"التكامل مع نقاط البيع",status:"لم يبدأ",assignee:"أحمد محمد",updates:[],createdAt:"",updatedAt:""},
          ]
        },
      ],
    },
    {
      id:"sp2", title:"منصة إدارة عملاء المؤسسات CRM",
      status:"جاري التنفيذ", progress:62, deadline:"2025-10-31",
      resources:[
        {id:"r3",name:"كريم عمر",role:"محلل أعمال",allocation:100},
        {id:"r4",name:"ليلى حسن",role:"مطور",allocation:100},
      ],
      components:[
        { id:"c4", driver:"challenge",
          title:"تأخر التكامل مع أنظمة الشركاء 3 أشهر",
          desc:"غياب توثيق موحّد لـ APIs أعاق التوصيل مع 4 شركاء رئيسيين في 2024",
          priority:"عالية", refYear:2024,
          tasks:[
            {id:"t6",title:"بناء بوابة API موحّدة",status:"جاري التنفيذ",assignee:"ليلى حسن",updates:[],createdAt:"",updatedAt:""},
            {id:"t7",title:"توثيق Swagger شامل",status:"مكتمل",assignee:"كريم عمر",updates:[],createdAt:"",updatedAt:""},
          ]
        },
        { id:"c5", driver:"regulatory",
          title:"لوائح الفواتير الإلكترونية الإلزامية 2025",
          desc:"القرار الوزاري يُلزم كل المؤسسات بالتحول للفوترة الإلكترونية بحلول يوليو 2025",
          priority:"عالية", refYear:2025,
          tasks:[
            {id:"t8",title:"تطوير وحدة الفوترة الإلكترونية",status:"جاري التنفيذ",assignee:"ليلى حسن",updates:[],createdAt:"",updatedAt:""},
            {id:"t9",title:"الاختبار مع هيئة الزكاة",status:"لم يبدأ",assignee:"كريم عمر",updates:[],createdAt:"",updatedAt:""},
          ]
        },
        { id:"c6", driver:"opportunity",
          title:"نمو قطاع الخدمات المؤسسية 34% سنوياً",
          desc:"تقرير IDC 2024: السوق المحلي يتضاعف كل 3 سنوات، ولدينا ميزة تنافسية في التكامل",
          priority:"متوسطة", refYear:2024,
          tasks:[
            {id:"t10",title:"وحدة الذكاء التجاري BI",status:"لم يبدأ",assignee:"كريم عمر",updates:[],createdAt:"",updatedAt:""},
          ]
        },
      ],
    },
    {
      id:"sp3", title:"البنية التحتية السحابية",
      status:"جاري التنفيذ", progress:72, deadline:"2025-08-31",
      resources:[
        {id:"r5",name:"محمد خالد",role:"مطور",allocation:100},
        {id:"r6",name:"هدى ناصر",role:"مختبر",allocation:80},
      ],
      components:[
        { id:"c7", driver:"challenge",
          title:"تجاوز ميزانية البنية التحتية 22%",
          desc:"غياب مراقبة التكاليف السحابية أدى لإنفاق غير مبرر في Q3 وQ4 من 2024",
          priority:"عالية", refYear:2024,
          tasks:[
            {id:"t11",title:"تطبيق منهجية FinOps",status:"جاري التنفيذ",assignee:"محمد خالد",updates:[],createdAt:"",updatedAt:""},
            {id:"t12",title:"لوحة مراقبة التكاليف",status:"مكتمل",assignee:"هدى ناصر",updates:[],createdAt:"",updatedAt:""},
          ]
        },
        { id:"c8", driver:"opportunity",
          title:"شراكة مع موفر سحابي تخفّض التكاليف 40%",
          desc:"عرض رسمي من AWS يتضمن خصومات committed use + دعم تقني مخصّص",
          priority:"عالية", refYear:2024,
          tasks:[
            {id:"t13",title:"ترحيل الخدمات الأساسية",status:"جاري التنفيذ",assignee:"محمد خالد",updates:[],createdAt:"",updatedAt:""},
            {id:"t14",title:"إعداد بيئة Multi-region",status:"لم يبدأ",assignee:"محمد خالد",updates:[],createdAt:"",updatedAt:""},
          ]
        },
        { id:"c9", driver:"strategic",
          title:"توجيه الإدارة: الاستقلالية التقنية الكاملة",
          desc:"قرار مجلس الإدارة بالتخلص من الاعتماد على موردَين للبنية التحتية بحلول 2026",
          priority:"متوسطة", refYear:2025,
          tasks:[
            {id:"t15",title:"أكاديمية الكفاءات السحابية الداخلية",status:"لم يبدأ",assignee:"هدى ناصر",updates:[],createdAt:"",updatedAt:""},
          ]
        },
      ],
    },
...
function recalcSP(sp: SubPlan): SubPlan {
  const allTasks = sp.components.flatMap(c => c.tasks);
  if (!allTasks.length) return sp;
  const done = allTasks.filter(t => t.status === "مكتمل").length;
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"#000d", zIndex:200,
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        padding:22, width:"100%", maxWidth:wide?680:450,
...
function DriverTag({ driver, refYear }: { driver: string; refYear?: number }) {
  ...
  return (
    <span style={{
      ...
      border:`1px solid ${d.color}35`, borderRadius:20,
...
function UpdateLogModal({ task, onAddUpdate, onClose }: {
  task: Task;
  onAddUpdate: (tid: string, note: string) => void;
  onClose: () => void;
}) {
  const [text, setText] = useState("");
  const logs: UpdateLog[] = task.updates || [];

  const submit = () => {
    if (!text.trim()) return;
    onAddUpdate(task.id, text.trim());
    setText("");
  };
  ...
  return (
    <Modal title={`سجل التحديثات — ${task.title}`} onClose={onClose} wide>
      ...
      <div style={{ borderTop:`1px solid ${C.border}`, paddingTop:16 }}>
        <div style={{ fontSize:12, color:C.sub, marginBottom:12, fontWeight:700 }}>
          السجل ({logs.length} تحديث)
        </div>
        {logs.length === 0
            ...
            <div key={i} style={{
              ...
              <div style={{ flex:1, minWidth:0 }}>
                ...
                <div style={{ fontSize:12, color:C.text, lineHeight:1.7,
                  background:C.raised, borderRadius:7, padding:"8px 10px" }}>
                  {log.note}
                </div>
              </div>
            </div>
...
function ComponentCard({ comp, spId, onUpdateComp, onDeleteComp }: {
  comp: Comp; spId: string;
  onUpdateComp: (spId: string, updated: Comp) => void;
  ...
  const setTaskStatus = (tid: string, status: string) => {
    const ts = nowISO();
    const tasks = comp.tasks.map(t => {
      if (t.id !== tid) return t;
      const autoNote = `تغيير الحالة: ${t.status} ← ${status}`;
      const newLog = { at:ts, note:autoNote, auto:true };
      return { ...t, status, updatedAt:ts, updates:[...(t.updates||[]), newLog] };
    });
    onUpdateComp(spId, { ...comp, tasks });
  };

  const addUpdate = (tid: string, note: string) => {
    const ts = nowISO();
    const tasks = comp.tasks.map(t => {
      if (t.id !== tid) return t;
      const newLog = { at:ts, note };
      return { ...t, updatedAt:ts, updates:[...(t.updates||[]), newLog] };
    });
    onUpdateComp(spId, { ...comp, tasks });
  ...
  const deleteTask = (tid: string) => {
    onUpdateComp(spId, { ...comp, tasks:comp.tasks.filter(t => t.id !== tid) });
  };
  ...
  const submitTask = () => {
    ...
    const task: Task = {
      ...
      updates:[{ at:ts, note:"تم إنشاء المهمة", auto:true }],
    };
    onUpdateComp(spId, { ...comp, tasks:[...comp.tasks, task] });
        ...
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", gap:8, flexWrap:"wrap" }}>
            ...
            <div style={{ display:"flex", gap:6, alignItems:"center", flexShrink:0 }}>
              ...
              <Btn onClick={e => { e?.stopPropagation(); onDeleteComp(spId, comp.id); }} sm variant="danger">✕</Btn>
            </div>
          </div>
...
// ─── Component Form ───────────────────────────────────────────────────────────
function ComponentForm({ initial, spId, onSave, onClose }: {
  initial?: Comp; spId: string;
  onSave: (spId: string, comp: Comp) => void;
  onClose: () => void;
}) {
  const blank: Comp = { id:uid(), driver:"challenge", title:"", desc:"", priority:"عالية", refYear:2024, tasks:[] as Task[] };
  const [f, setF] = useState<Comp>(initial || blank);

  return (
    <Modal title={initial?"تعديل العنصر":"إضافة تحدٍّ / فرصة / مبادرة"} onClose={onClose} wide>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
          ...
          <div>
            ...
            <Sel value={f.refYear} onChange={v => setF(p => ({ ...p, refYear:+v }))}
              options={[2023,2024,2025].map(y => [y, String(y)] as [number, string])}/>
          </div>
        </div>

        <div style={{ display:"flex", gap:8, justifyContent:"flex-end" }}>
          <Btn onClick={onClose} variant="ghost" sm>إلغاء</Btn>
          <Btn onClick={() => { if (f.title.trim()) { onSave(spId, f); onClose(); } }} active sm>حفظ</Btn>
        </div>
      </div>
    </Modal>
  );
...
function SubplanDetail({ sp, onUpdateSP }: { sp: SubPlan; onUpdateSP: (updated: SubPlan) => void }) {
  ...
  const updateComp = useCallback((_spId: string, updated: Comp) => {
    const components = sp.components.map(c => c.id === updated.id ? updated : c);
    onUpdateSP(recalcSP({ ...sp, components }));
  }, [sp, onUpdateSP]);

  const deleteComp = useCallback((_spId: string, cid: string) => {
    const components = sp.components.filter(c => c.id !== cid);
    onUpdateSP(recalcSP({ ...sp, components }));
  }, [sp, onUpdateSP]);

  const saveComp = (spId: string, comp: Comp) => {
    const existing = sp.components.find(c => c.id === comp.id);
    const components = existing
      ? sp.components.map(c => c.id === comp.id ? comp : c)
      : [...sp.components, comp];
    onUpdateSP(recalcSP({ ...sp, components }));
  };
  ...
  const submitRes = () => {
    ...
    onUpdateSP({ ...sp, resources:[...sp.resources, { id:uid(), ...newRes }] });
    setNewRes({ name:"", role:ROLES[0], allocation:100 });
    setAddRes(false);
  };

  const removeRes = (rid: string) => onUpdateSP({ ...sp, resources:sp.resources.filter(r => r.id !== rid) });

  const applyProg = () => {
    const p = Math.max(0, Math.min(100, +tmp));
    onUpdateSP({ ...sp, progress:p, status:p===100?"مكتمل":p>0?"جاري التنفيذ":"لم يبدأ" });
    setEditProg(false);
  };
  ...
  return (
    <div>
      <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:13,
        ...
        <div style={{ flex:1, minWidth:160 }}>
          ...
          <div style={{ display:"flex", gap:10, marginTop:7, flexWrap:"wrap", alignItems:"center" }}>
            <span style={{
              background:cfg.bar, color:cfg.color, border:`1px solid ${cfg.color}35`,
      ...
      {addComp && <ComponentForm spId={sp.id} onSave={saveComp} onClose={() => setAddComp(false)}/>}
    </div>
  );
}
          ...
          return (
            <HoverCard key={sp.id} onClick={() => onSelect(sp.id)} style={{ marginBottom:18 }}>
              ...
              <div style={{ marginTop:10, background:C.muted, borderRadius:99, height:7, overflow:"hidden" }}>
                <div style={{ height:"100%", width:`${sp.progress}%`, background:cfg.color,
...
// ─── Add Subplan Modal ────────────────────────────────────────────────────────
function AddSPModal({ onAdd, onClose }: { onAdd: (sp: SubPlan) => void; onClose: () => void }) {
    ...
    <Modal title="إضافة خطة فرعية جديدة" onClose={onClose}>
      <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
        ...
        <div style={{ display:"flex", gap:8, justifyContent:"flex-end" }}>
          <Btn onClick={onClose} variant="ghost" sm>إلغاء</Btn>
          <Btn onClick={() => {
            if (f.title.trim() && f.deadline) {
              onAdd({ id:uid(), ...f, status:"لم يبدأ", progress:0, resources:[] as Resource[], components:[] as Comp[] });
              onClose();
            }
          }} active sm>إضافة</Btn>
        </div>
...
function SnapshotCard({
  snap, selected, onSelect, onDelete, onExport,
}: {
  snap: Snapshot;
  selected: boolean;
  onSelect: () => void;
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:8 }}>
        <div style={{ flex:1, minWidth:0 }}>
          ...
          <div style={{ fontSize:11, color:C.muted, marginTop:3 }}>
            ...
            <span style={{
              marginRight:8, background:`${C.accent}18`, color:C.accent,
              border:`1px solid ${C.accent}30`, borderRadius:5,
              padding:"1px 7px", fontSize:10,
            }}>{periodLabels[snap.period] || snap.period}</span>
          </div>
        </div>
        <div style={{ display:"flex", gap:6, flexShrink:0 }}>
          <button onClick={e => { e.stopPropagation(); onExport(); }} title="تصدير JSON"
...
function ReportsView({ project, snapshots, onAddSnapshot, onDeleteSnapshot }: {
  ...
  const handleExportAll = () => {
    exportJSON({ project, snapshots, exportedAt: nowISO() }, `dt-report-${Date.now()}.json`);
  };

  const handleExportSnap = (snap: Snapshot) => {
    exportJSON(snap, `snapshot-${snap.id}.json`);
  };
    ...
    <div>
      ...
      <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:13, padding:18, marginBottom:16 }}>
        ...
        <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
          ...
          {snapshots.length > 0 && (
            <Btn onClick={handleExportAll} variant="ghost">⬇ تصدير الكل JSON</Btn>
          )}
        </div>
      </div>
        ...
        {snapshots.length === 0
          ...
          : [...snapshots].reverse().map(snap => (
            <SnapshotCard
              ...
              onSelect={() => {
                if (!compareA || compareA === snap.id) setCompareA(snap.id);
                else setCompareB(snap.id);
              }}
              onDelete={() => onDeleteSnapshot(snap.id)}
              onExport={() => handleExportSnap(snap)}
            />
          ))
        }
      </div>
...
export default function App() {
  ...
  const updateSP = useCallback((updated: SubPlan) => {
    setProject(prev => ({ ...prev, subplans:prev.subplans.map(sp => sp.id === updated.id ? updated : sp) }));
  }, []);

  const addSP = (sp: SubPlan) => setProject(prev => ({ ...prev, subplans:[...prev.subplans, sp] }));

  const removeSP = (id: string) => {
    setProject(prev => ({ ...prev, subplans:prev.subplans.filter(sp => sp.id !== id) }));
    if (selId === id) { setSelId(null); setView("overview"); }
  };

  const handleAddSnapshot = (label: string, period: Snapshot["period"]) => {
    const snap = createSnapshot(
      project,
      label || `لقطة ${new Date().toLocaleDateString("ar-SA", { day:"2-digit", month:"short", year:"numeric" })}`,
      period,
    );
    ...
    <div style={{ minHeight:"100vh", background:C.bg, color:C.text,
      ...
      <div style={{ background:C.surface, borderBottom:`1px solid ${C.border}`,
        ...
        <div style={{ display:"flex", gap:8 }}>
          <Btn onClick={() => { setView("overview"); setSelId(null); }}
            active={view === "overview" && !selId} sm>📊 اللوحة</Btn>
          <Btn onClick={() => setView("reports")}
            active={view === "reports"} sm>📋 التقارير</Btn>
        </div>
      </div>
                ...
                <div style={{ display:"flex", gap:8, alignItems:"flex-start" }}>
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    ...
                    <div style={{ display:"flex", gap:4, marginTop:4 }}>
                      {dTags.map(k => (
                        <span key={k} style={{ fontSize:10, color:DRIVER[k].color }}>{DRIVER[k].icon}</span>
                      ))}
                    </div>
